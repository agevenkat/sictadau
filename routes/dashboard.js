  ...
  try {
    ...
    const results = await db.batch([
      ...
      { sql: `SELECT COUNT(*) as total,
        ...
        FROM vouchers v JOIN projects p ON v.project_id = p.id WHERE p.project_type = ?`, args: [medium] },
      ...
      { sql: `SELECT COUNT(*) as total,
        ...
        FROM projects WHERE project_type = ?`, args: [medium] },
      ...
      { sql: `SELECT SUM(CASE WHEN s.amount_type='Credit' THEN s.amount ELSE 0 END) as total_credit,
        ...
        WHERE (s.project_id IS NULL OR p.project_type = ?)`, args: [medium] },
      // recent vouchers
      { sql: `SELECT v.id, v.status, v.final_amount, v.created_at,
        m.full_name, p.film_name FROM vouchers v
        JOIN members m ON v.member_id = m.id
        JOIN projects p ON v.project_id = p.id
        WHERE p.project_type = ? ORDER BY v.created_at DESC LIMIT 8`, args: [medium] },
      // recent projects
      { sql: `SELECT p.id, p.film_name, p.production_company, p.amount, p.status, p.end_date
        FROM projects p WHERE p.project_type = ? ORDER BY p.created_at DESC LIMIT 5`, args: [medium] },
      // pending vouchers
      { sql: `SELECT v.id, v.final_amount, m.full_name, m.bank_account_no, p.film_name
        ...
        JOIN projects p ON v.project_id = p.id
        WHERE v.status = 'Pending' AND p.project_type = ? ORDER BY v.created_at LIMIT 10`, args: [medium] },
      ...
      { sql: `SELECT strftime('%Y-%m', COALESCE(v.voucher_date, date(v.created_at))) as month,
        ...
        WHERE p.project_type = ? GROUP BY month ORDER BY month DESC LIMIT 12`, args: [medium] },
      ...
      { sql: `SELECT strftime('%Y-%m', s.transaction_date) as month,
        ...
        FROM statements s LEFT JOIN projects p ON s.project_id = p.id
        WHERE (s.project_id IS NULL OR p.project_type = ?)
